...
<style>
  ...
  .container::before {
    ...
    width: 100%;
    height: 100%;
    background-image: url(https://github.com/KenNg-web/KenNg-web.github.io/blob/main/BR%20Logo.jpg);
...
function renderInputs(id){
  ...
  if(type === 'Mortgage'){
    ...
    html += `</div>`;
    html += `<label>Service Fees (%): </label><input id="fees${id}" type="text" placeholder="Base 14%, Max 20%">`;
    html += `<div class="row">`;
    ...
    if(mode === 'loan'){
      html += `<label>Loan Size (RM)</label><input id="loan${id}" type="text" placeholder="If Above 200k Refer To Keyman.">`;
    } else {
      html += `<label>Monthly Payment (RM)</label><input id="monthly${id}" type="text" placeholder="Follow Client Affordable Payment.">`;
    }
    html += `<div class="row">`;
    ...
    html += `</div>`;
    html += `<div class="remark">Service fees not applied to Hire Purchase</div><input id="fees${id}" type="hidden">`;
